...
export async function rateLimit(key:string,max:number,seconds:number,message='Too many attempts. Please wait before trying again.'){const allowed=await rpc<boolean>('slotpilot_rate_limit',{p_namespace:config().namespace,p_key:key,p_limit:max,p_seconds:seconds});if(!allowed)throw new AppError('RATE_LIMIT',message,429)}
