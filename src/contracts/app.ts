...
export const CatalogSchema = v.object({ services:v.array(ServiceSchema,{max:8}), vehicles:v.array(DemoVehicleSchema,{max:6}), branches:v.array(BranchSchema,{max:2}), dates:v.array(DateSchema,{max:30}), clock:v.object({date:DateSchema,time:TimeSchema}), demoData:v.literal(true) });
