...
Зарегистрируйтесь по email и паролю (от 8 символов), подтвердите почту и войдите. Записи связаны с учётной записью Supabase Auth и доступны после повторного входа. Старые анонимные демо-записи автоматически не переносятся. Для `/admin` и `/demo` у владельца должен быть `role: "admin"` в **Supabase Auth → Users → App Metadata**. Пользователь должен выйти и снова войти; обычная регистрация всегда даёт права клиента. Старые общие демо-пароли больше не применяются.
