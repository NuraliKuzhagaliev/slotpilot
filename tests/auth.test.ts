...
import { currentUser, login, resendConfirmation, signUp } from '../src/server/app/auth.ts';
...
test('personal sign-in preserves identity and only trusted app metadata grants admin', async () => {
  ...
  let role = 'client', signupBody: Record<string, unknown> | null = null, resendBody: Record<string, unknown> | null = null;
  globalThis.fetch = async (input, init) => {
    ...
    if (url.endsWith('/resend?type=signup')) { resendBody = JSON.parse(String(init?.body)); return Response.json({}); }
  ...
  try {
    ...
    await resendConfirmation(request, 'someone@example.com');
    assert.deepEqual(resendBody, { email: 'someone@example.com' });
