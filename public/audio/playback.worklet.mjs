...
class PlaybackProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this.port.onmessage = ({ data }) => {
      if (data === 'clear') {
        // Prefer the continuous queued waveform; a constant-sample fade creates a thump.
        const length = Math.min(this.ring.available, Math.ceil(sampleRate * 0.008));
        const tail = new Float32Array(length || Math.ceil(sampleRate * 0.002));
        if (length) { this.ring.pull(tail); for (let i = 0; i < tail.length; i++) tail[i] *= 0.75; }
        else tail.fill(this.lastSample);
  ...
  process(inputs, outputs) {
    ...
    if (count) {
      ...
      // Play every packet immediately. Only ease the first 2 ms after actual silence.
      for (let i = 0; i < count; i++) {
        this.ramp = Math.min(1, this.ramp + 1 / this.rampSamples);
        chunk[i] *= 0.75 * this.ramp;
      }
      this.lastSample = chunk[count - 1]; this.wasPlaying = true; this.silenceFrames = 0;
    }
    if (count < output[0].length) {
      // A packet ran out: ease its last sample to zero before the next arrives.
